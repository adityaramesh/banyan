...
to made in one atomic transaction. The `Mongo` data layer provided by Eve doesn't implement this
functionality. Instead, we use the following steps:
...
be invalid if it is too large. To ensure that this doesn't happen, we restrict the sizes of the
following:
...
the constants as defined below, the maximum update size is precisely 16 Mb. Atomic updates larger
...
"""
We also put a cap on the maximum number of times a task can be retried, so that the execution
